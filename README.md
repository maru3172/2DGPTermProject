...
handle_event(): 전역 이벤트 처리

키 1을 누르면 world의 objects 출력
...
clamp(min_value, value, max_value): 값을 주어진 범위 내로 제한

최소값, 현재값, 최대값을 받아서 범위 내의 값 반환

Enemy 클래스의 레벨 제한과 Fighter의 이동 제한에 사용


check_game_over(): 게임 오버 상태 체크

목숨이 0이 되면 게임 오버 처리

lives_sprite와 main_scene_ui의 게임 오버 상태 설정
